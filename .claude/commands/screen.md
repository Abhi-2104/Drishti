...
2. Market-driven discovery when the criteria are momentum- or flow-shaped: get_top_gainers /
   get_top_losers / get_most_active (movers by % and by value), get_nifty_indices (sector
   rotation — which sectors lead/lag), get_top_bulk_buys / get_top_bulk_sells (what large
   clients are accumulating or distributing).
